...
def drop_duplicates(df: pd.DataFrame, subset: List[str] = None) -> pd.DataFrame:
    ...
    # Extract subset columns as a numpy array for efficient access
    arr = df[subset].to_numpy()
    seen = set()
    keep_indices = []
    # Iterate over rows using the numpy array
    for i, values in enumerate(map(tuple, arr)):
